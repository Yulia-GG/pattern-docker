...
`Docker Compose является мощным инструментом для управления многоконтейнерными приложениями. Он позволяет быстро создавать и управлять несколькими контейнерами как одной целой системой, что значительно упрощает процесс развертывания и масштабирования приложений. Docker Compose включает в себя следующие преимущества:`
...
`Создаем файл docker-compose.yml и вносим туда первичные настройки:

version;
services;
volumes;
networks.
При выполнении задания используется подсеть 10.5.0.0/16. Ваша подсеть должна называться: <ваши фамилия и инициалы>-my-netology-hw. Все приложения из последующих заданий должны находиться в этой конфигурации.`
...
Добавляем необходимые тома с данными и конфигурацией. Добавляем переменную окружения с путем до файла с кастомными настройками.
Обеспечиваем внешний доступ к порту 3000 c порта 80 докер-сервера.`
